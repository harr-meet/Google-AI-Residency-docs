...
def make_train():
    ...
    for fish in folders: 
        ...
        for i in fish_path:
            fish_mtx = img_read(i)
            fish_mtx = img_threshold_otsu(fish_mtx)
...
#Function to returning images under Otsu binarization
def img_threshold_otsu(img):
    grayscaled = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    _, threshold = cv2.threshold(grayscaled, 120, 260, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    return threshold
...
predsFrame.to_csv('submissionOtsu.csv',index=False)
